#This Terraform Code Deploys Basic VPC Infra.t
provider "aws" {
...
resource "aws_route_table_association" "routetable-associ" {
    count = 3 #0,1,2
    subnet_id = "${element(aws_subnet.subnets.*.id, count.index)}"
    route_table_id = "${element(aws_route_table.routetable-associ.id, count.index)}"
}
